function [vertex, faces, normal, vertex_number, faces_number] = load_BV_mesh(filename)
...
if nargin==0
    [f, pathname] = uigetfile({'*.mesh'},'Pick a file');
    filename = [pathname,f];
end
...
switch char(file_format)'
	...
	case 'ascii'
		...
		for i=1:mesh_time
			...
			fcs = fscanf(fid,'(%d ,%d) ',polygon_dimension*faces_number);
			faces{i} = reshape(fcs, polygon_dimension, faces_number)';
		end
end
