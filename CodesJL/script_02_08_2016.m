...
if hemi == 'rh'
    ...
    vertex_coords=vertex_coords./repmat(sqrt(sum(vertex_coords.^2,2)),1,3);
    faces=faces(:,[1 3 2]);
    write_surf(100*vertex_coords,faces,name1)
    
    name2=['/Users/ghfc/Documents/Dropbox/hugo_is_dead/scripts/BrainCatalogueWorkflow/results/', content(i2).name, '/surf/rh.sphere'];
   
    [vertex_coords, faces] = read_surf(name2);
    vertex_coords=vertex_coords-repmat(mean(vertex_coords,1),length(vertex_coords),1);
    vertex_coords=vertex_coords./repmat(sqrt(sum(vertex_coords.^2,2)),1,3);
    faces=faces(:,[1 3 2]);
    write_surf(100*vertex_coords,faces,name2)
else
